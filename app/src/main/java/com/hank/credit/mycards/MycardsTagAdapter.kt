...
class MycardsTagAdapter(val viewModel: MycardsViewModel) : ListAdapter<String, MycardsTagAdapter.tagViewHolder>(TagDiffCallback()){
    ...
    class tagViewHolder(val binding: ListSearchTagBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(item: String, viewModel: MycardsViewModel){
            binding.name = item
...
class TagListener(val type: String, val viewModel : MycardsViewModel): CompoundButton.OnCheckedChangeListener{
    override fun onCheckedChanged(buttonView: CompoundButton?, isChecked: Boolean) {
        if(isChecked){
            viewModel.onMyTagClicked(type)
        }else{
            viewModel.initializeCardCoupon()
        }
    }
}